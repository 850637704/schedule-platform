 ...
 */
function computeSwapCandidates(entries, source, weekType, meetings, teacherMap) {
  // 找到源课程条目
  ...
  for (const target of sameClassEntries) {
    ...
    const result = checkSwappable(entries, sourceEntry, target, weekType, meetings, teacherMeetings);
    candidates.push({
      class: target.class,
 ...
 */
function checkSwappable(entries, source, target, weekType, meetings, teacherMeetings) {
...
function executeSwap(entries, source, target, weekType, meetings, teacherMap) {
  ...
  // 最终校验
  const teacherMeetings = buildTeacherMeetings(teacherMap);
  const check = checkSwappable(entries, entries[sIdx], entries[tIdx], weekType, meetings, teacherMeetings);
